describe('Reports', function () {
  ...
  i++;
  it(i + ' - Choose the row to Reports per Equipment table to see more charts', function () {
    cy.get('tr').contains('ED2011-051 (Zebra 170Xi4 Label System - Boxing)').click();
    cy.wait(1000);
  });
  i++;
  it(i + ' - Show Events for ED', function () {
    cy.wait(1000);
    cy.request({
      method: 'POST',
      url: `eredtag/php/queries/get_scattered.php`
    }).its('body').should('not.be.empty');
  });
  i++;
  it(i + ' - Show Empirical Distribution Function for ED', function () {
    cy.wait(1000);
    cy.request({
      method: 'POST',
      url: `eredtag/php/queries/get_empirical_dist.php`
    }).its('body').should('not.be.empty');
  });
  i++;
  it(i + ' - Show Mean Time Between Failures for ED', function () {
    cy.wait(1000);
    cy.request({
      method: 'POST',
      url: `eredtag/php/queries/get_mtbf.php`
    }).its('body').should('not.be.empty');
  });
});
