...
it('0' + i + ' - Select Label Printing Station', function () {
  cy.get('#station').select('Label Printing');
});
i++;
...
it('0' + i + ' - Scan an order already entered in the scrap section', function () {
  cy.get('#scrap').type('1180959_10517513', { delay: 200 });
...
it(i + " - Display succesfully message 'Record saved!'", function () {
  ...
  cy.wait(1000);
  cy.get('button').contains('OK').click();
  cy.get('#station').should('contain', "Label Printing");
  cy.wait(1000);
});
i++;
...
it(i + ' - Scan production order not scanned in previous station', function () {
  cy.get('#productionOrder').clear();
  cy.get('#productionOrder').type('1180954_10517508', { delay: 200 });
  cy.get('#productionOrder').should('be.enabled');
  cy.get('#productionOrder').type('{enter}');
  cy.wait(1000);
});
...
it(i + ' - Scan an order already entered in the rework section', function () {
  cy.get('#rework').type('1180981_10517515', { delay: 200 });
...
it(i + ' - Scan order in rework', function () {
  cy.get('#rework').clear();
  cy.get('#rework').type('1180989_10517523', { delay: 200 });
  cy.get('#rework').should('be.enabled');
  cy.get('#rework').type('{enter}');
  cy.wait(1000);
});
